...
namespace pp
{
    class DBf
    {
        ...
        // pc
        static string conn = @"Data Source=DESKTOP-6ELC94T\SQLEXPRESS;Initial Catalog=pp;Integrated Security=True";
        // not
        //static string conn = @"Data Source=LAPTOP-NK04PI6P;Initial Catalog=pp;Integrated Security=True";

        static public bool Login(string log, string pas, Form frm)
        {
        ...
        static public bool Register(string log, string pas, string F, string I, string O, string mail, string phone, string ier)
        {
            con = new SqlConnection(conn);
            try
            {
        ...
        static public string recalculateTime(string org_time, string add_time)
        {
            if (org_time == "")
                org_time = "00:00:00";
            if (add_time == "")
        ...
        static public void rewindDay(int cout_day)
        {
            ...
            try
            {
                for (int q = 1; q <= cout_day; q++)
                {
                    ...
                    cmd.ExecuteNonQuery();
                    cmd = new SqlCommand($"UPDATE tab SET day1 = '{DateTime.Now + (q - 1).ToString("dd.MM.yyyy")}' WHERE user_ID = '0'", con);
                    cmd.ExecuteNonQuery();
                }
            }
            catch (Exception ex)
        ...
        static public string updateTotalTime()
        {
            string total;
            string time_day;
        ...
        static public bool delWorkers(string ID)
        {
            con = new SqlConnection(conn);
            con.Open();
            try
